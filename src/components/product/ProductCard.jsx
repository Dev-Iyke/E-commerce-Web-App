    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-y-8 gap-x-6">
      {allProducts.map((product) => {
        return (
          <div
            key={product.id}
            className=" pt-4 rounded-lg shadow-lg transition-transform transform hover:scale-105 duration-300 bg-white"
          >
            <div className="px-4">
              <img
                ...
                alt={product.title}
              />
              <h2 className="text-lg font-semibold mb-2 line-clamp-1 dark:text-[#333]">
                {product.title}
              </h2>
              <p className="text-sm text-gray-400 line-clamp-3">
                {product.description}
              </p>
              <h4 className="dark:text-[#333]">
